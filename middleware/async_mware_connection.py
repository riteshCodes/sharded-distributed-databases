...
class Listener(CommunicationServiceServicer):
    ...
    def getSingle(self, request, context):
        ...
        value
        """
        value = asyncio.run(m_ware.get_single(key_list=[request.key]))
    ...
    def setSingle(self, request, context):
        ...
        """
        asyncio.run(m_ware.set_to(key=request.userID, name=request.name, email=request.email))
    ...
    def getMultiple(self, request, context):
        ...
        for k in request.key_list:
            keys.append(k.key)
        values = asyncio.run(m_ware.get_multiple(key_list=keys))
    ...
    def setMultiple(self, request, context):
        ...
        asyncio.run(m_ware.set_multiples(key_list=uid_list, name_list=name_list, email_list=email_list))
        return StringMessage(message='OK')
    ...
    def delSingle(self, request, context):
        ...
        deleted = asyncio.run(m_ware.del_single(key_list=keys))
        if deleted is None:
    ...
    def delMultiple(self, request, context):
        ...
        deleted = asyncio.run(m_ware.del_multiple(key_list=keys))
        if deleted is None:
            return StringMessage(message='NONE')
    ...
    def getRange(self, request, context):
        ...
        values_list = asyncio.run(m_ware.get_range(start=request.start, end=request.end))
