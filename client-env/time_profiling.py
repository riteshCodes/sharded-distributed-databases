...
FILE_PATH = path.join(path.dirname(Path(__file__)), Path('profiling/client_profiling_1_db.xlsx'))
OUTPUT_FOLDER = '1_DB_Config/comparison' if comparison else '1_DB_Config'
...
def visualize_data(*, function, sheet_name, with_columns):
    ...
    for col in df.columns:
        label_comp = ' (Without Middleware)' if 'Without_Middleware' in col else ''
        plt.plot(default_x_ticks, df[col], marker='*', label=f'{function}{label_comp}')
    ...
    plt.grid(True)
    plt.savefig(f'assets/{OUTPUT_FOLDER}/{function.split()[0].lower()}.pdf', dpi=1200)
    plt.savefig(f'assets/{OUTPUT_FOLDER}/{function.split()[0].lower()}.svg', dpi=1200)
    plt.show()
...
def visualize_data_combined_operations(*, with_columns):
    ...
    plt.savefig(f'assets/{OUTPUT_FOLDER}/combined_visualization.svg', dpi=1200)
    plt.show()
...
if __name__ == '__main__':
    ...
    if comparison:
        ...
        visualize_data(function='Ping (String) Request', sheet_name='Ping Message',
                       with_columns=['Response_Time (seconds)', 'Without_Middleware'])
